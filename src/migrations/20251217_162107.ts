...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE INDEX \`pages_blocks_service_header_order_idx\` ON \`pages_blocks_service_header\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`pages_blocks_service_header_parent_id_idx\` ON \`pages_blocks_service_header\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`pages_blocks_service_header_path_idx\` ON \`pages_blocks_service_header\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`pages_blocks_service_header_service_idx\` ON \`pages_blocks_service_header\` (\`service_id\`);`)
